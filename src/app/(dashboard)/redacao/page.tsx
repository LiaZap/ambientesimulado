        ...
        <div className="space-y-8 animate-in fade-in duration-500">
            ...
            {essays.length === 0 ? (
                ...
                <div className="grid gap-4">
                    {essays.map((essay) => (
                        <Link href={`/redacao/${essay.id}`} key={essay.id} className="block group">
                            <Card className="bg-slate-900 border-slate-800 text-slate-100 group-hover:border-yellow-500/50 transition-all">
                                <CardContent className="p-6 flex justify-between items-center">
                                    <div>
                                        <h3 className="font-bold text-lg mb-1 line-clamp-1 text-slate-200 group-hover:text-yellow-500 transition-colors">{essay.theme}</h3>
